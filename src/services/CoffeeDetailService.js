const CoffeeDetailDAO = require('../dao/CoffeeDetailDAO');
const CoffeeDetail    = require('../models/CoffeeDetail');

class CoffeeDetailService {
  constructor() {
    this.dao = new CoffeeDetailDAO();
  }

  async createDetail(data) {
    const detail = new CoffeeDetail(data);
  ...
  async getDetailById(drinkId) {
    ...
    if (!doc) throw new Error('CoffeeDetail not found');
    return doc;
  }
...
module.exports = CoffeeDetailService;
